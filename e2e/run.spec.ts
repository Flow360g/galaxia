    ...
    if (index === 0) {
      const intel = page.getByTestId("request-intel");
      {
        ...
        for (let i = 0; i < 25; i += 1) {
          const b = await intel.boundingBox();
          boxes.push(b ? `${b.x.toFixed(1)},${b.y.toFixed(1)},${b.width.toFixed(1)},${b.height.toFixed(1)}` : "null");
          await page.waitForTimeout(40);
        }
        console.log("PROBE " + JSON.stringify([...new Set(boxes)]));
      }
