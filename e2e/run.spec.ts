...
test("a small count is scored in whole numbers, not in percent", async ({ page }) => {
  await page.goto("/play?replay=1&round=2026-09-27");
  const toast = page.getByTestId("toast");
  ...
  for (const index of [0, 1]) {
    await expect(page.getByTestId("question")).toBeVisible({ timeout: 20_000 });
    await readUp(page);
    const question = countingRound.questions[index] as { answers: number[] };
  ...
  // A guitar has 6 strings on a 1..20 slider, so the aim moves a whole string
  // at a time: dropped between two, it lands on one, and the readout is the
  // number that gets scored.
  await expect(page.getByTestId("question")).toBeVisible({ timeout: 15_000 });
  await page.getByTestId("aim").fill("340");
  await expect(page.getByTestId("aim-value")).toHaveText("7 strings");

  // One string out. As a share of the truth that is 17% and used to cost 25
  // points, a shield and the streak; it is a close answer and is paid as one.
  await page.getByTestId("lock").click();
  await expect(toast).toHaveAttribute("data-outcome", "thread", { timeout: 10_000 });
  await expect(toast).toContainText("CLOSE");
  await expect(page.getByTestId("toast-points")).toHaveText(/^\+100 POINTS$/);
  await expect(page.getByTestId("wide-by")).toHaveText("1 off");
  await expect(page.getByTestId("shield")).toHaveAttribute("data-shields", "3");
  await expect(page.getByTestId("damage")).toHaveCount(0);
